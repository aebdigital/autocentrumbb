    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {BOXES.map(({ type, label }) => (
          <button
            ...
            className="relative h-20 rounded-lg text-left px-4 cursor-pointer transition-all duration-300 bg-white hover:scale-[1.02]"
            style={
              active === type
                ? {
                    boxShadow: '0 0 0 2.5px #D01717, 0 8px 20px rgba(208,23,23,0.25)',
                    transform: 'scale(1.02)',
                  }
                : { boxShadow: '0 4px 12px rgba(0,0,0,0.12)' }
            }
          >
            <span className="absolute bottom-3 left-4 font-bold font-jakarta text-[22px] leading-tight text-gray-900">
              {label}
            </span>
          </button>
        ))}
      ...
      {active && active !== 'ev' && (
        <div className="mt-8 space-y-6">
          <div>
            <label className="block font-bold font-jakarta text-xl text-gray-900 mb-2">
      ...
      {/* Price result */}
      {price !== null && (
        <div className="mt-8 font-jakarta text-xl text-gray-900 font-bold">
