...
const STORAGE_KEY = 'autocentrumbb-cookie-preferences';
...
function CookieToggle({
  ...
  return (
    <button
      ...
      className={`relative h-7 w-14 rounded-full transition ${checked ? 'bg-red-600' : 'bg-gray-300'} ${disabled ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}
    >
      <span
    ...
    () => [
      {
        ...
        description: 'Zabezpečujú základnú funkčnosť stránky a nie je možné ich vypnúť.',
        disabled: true,
      },
      {
        key: 'functional' as const,
        title: 'Funkčné cookies',
        description: 'Pomáhajú si zapamätať vaše voľby a zlepšujú komfort pri používaní webu.',
      },
      {
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-4xl">
                ...
                <p className="font-montserrat text-sm leading-7 text-gray-600">
                  ...
                  <button type="button" onClick={() => setShowSettings(true)} className="font-semibold text-red-600 underline underline-offset-4">
                    Nastavenia
                  </button>{' '}
                  si môžete upraviť svoje preferencie. Viac informácií nájdete na stránke{' '}
                  <Link href="/ochrana-osobnych-udajov" className="font-semibold text-red-600 underline underline-offset-4">
        ...
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-5">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.25em] text-red-600">Nastavenia</p>
                <h3 className="mt-1 font-jakarta text-2xl font-bold text-gray-900">Nastavenia cookies</h3>
              </div>
              <button type="button" onClick={() => setShowSettings(false)} className="rounded-full border border-gray-200 px-3 py-1 text-2xl leading-none text-gray-500 transition hover:border-gray-300 hover:text-gray-900">
                ×
              </button>
            </div>
            ...
            <div className="space-y-4 px-6 py-6">
              {rows.map((row) => (
                <div key={row.key} className="flex items-start justify-between gap-4 rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4">
                  <div className="pr-4">
                    <h4 className="font-jakarta text-lg font-bold text-gray-900">{row.title}</h4>
                    <p className="mt-1 font-montserrat text-sm leading-6 text-gray-600">{row.description}</p>
                  </div>
                  <CookieToggle
                    ...
                    onChange={
                      row.disabled
                        ? undefined
                        : () => setPreferences((current) => ({ ...current, [row.key]: !current[row.key] }))
                    }
                  />
                </div>
              ))}
            ...
            <div className="flex flex-col gap-3 border-t border-gray-200 px-6 py-5 sm:flex-row sm:justify-end">
              ...
              <button type="button" onClick={saveCurrentSettings} className="whitespace-nowrap rounded-xl border border-red-200 bg-red-50 px-5 py-3 font-jakarta text-sm font-bold text-red-700 transition hover:bg-red-100">
                Uložiť nastavenia
              </button>
